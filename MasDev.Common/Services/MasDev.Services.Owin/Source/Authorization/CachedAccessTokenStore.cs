...
namespace MasDev.Services.Auth
{
	public class CachedAccessTokenStore : ICachedCredentialsRepository
	{
		...
		public async Task<DateTime?> GetlastInvalidationUtcAsync (int id, int flag)
		{
			...
			var lastInvalidation = await _persistentStoreFactory ().GetlastInvalidationUtcAsync (id, flag);
		...
		public async Task SetInvalidationTime (int id, int flag, DateTime invalidationTimeUtc)
		{
			...
			await _persistentStoreFactory ().SetInvalidationTime (id, flag, invalidationTimeUtc);
		}
